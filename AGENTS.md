...
- Response bodies that appear binary are not written to stdout when stdout is a terminal unless output is explicitly forced with `--output -`; this guard applies to both buffered formatting fallback output and raw streaming paths such as `--format off`. Raw streaming terminal output buffers and scans a bounded prefix before committing bytes so delayed binary/control data is caught before terminal writes.
