...
- Rust response bodies written to terminal stdout are routed through `less -FIRX` unless `--no-pager` or `no-pager = true` disables the pager; piped stdout and output-file writes bypass the pager.
