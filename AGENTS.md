...
- Formatted SSE, NDJSON, and gRPC stdout streaming share `src/http/mod.rs`'s formatter callback driver for decoded reads, clipboard capture, byte counting, trailer extraction, and flushes; keep per-format parsing inside the callbacks.
