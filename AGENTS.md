...
- ECH discovery validates HTTPS/SVCB ECHConfigLists, preserves DNS priority, applies `off|auto|on` policy before transport setup, rejects explicit TLS 1.2 bounds and hard ECH with HTTP/3, and does not silently ignore ECH through proxies or cleartext HTTP/2.
